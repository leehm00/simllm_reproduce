...
def parse_args():
    parser = argparse.ArgumentParser(description="Llama LSH Model Inference Script")
    ...
    parser.add_argument("--threshold", type=float, default=0.9, 
                       help="LSH similarity threshold (0-1)")
...
try:
    # 导入本地修改后的模型实现
    from models.modeling_llama_lsh import LlamaForCausalLM
    print("成功加载本地修改的Llama LSH模型代码！")
except ImportError as e:
    print(f"导入错误：{e}")
    print("请确认：")
    print("1. 当前目录存在 models/modeling_llama_lsh.py")
...
# 加载模型和分词器
def start_model(model_path, threshold, max_kv_size, eviction_mode):
    ...
    # 设置LSH相关参数
    if hasattr(model, 'set_lsh_args'):
        model.set_lsh_args(threshold, max_kv_size, eviction_mode)
...
def main():
    ...
    # 加载模型时传递参数
    model, tokenizer = start_model(model_path, threshold, max_kv_size, eviction_mode)
